...
- Mức prototype nhắm tới: [ ] Sketch [x] Mock [ ] Working — Mock: flow bấm được trọn (nhận câu hỏi → tra `kb/` → hiện kết quả), **AI thật ở lõi** (bước ③ gọi LLM 1 lần, trả JSON `label/doc_ids/reason/question/answer`); phần "gửi ticket thật cho TA" và nguồn `kb/` là tự dựng/mô phỏng, ghi rõ trong `codebase/README.md`.
