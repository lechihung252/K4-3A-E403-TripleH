...
Mục tiêu 5 người ngoài nhóm, trong đó 2 willing user đã khai từ CP1 (Nguyễn Vũ Anh, Nguyễn Duy Phong). Người thử tự gõ câu hỏi theo **tình huống** được giao, nhóm không đọc câu cho họ. Mỗi người 4 task, ứng với 4 đường đi ở `spec.md` §6.
...
*Mức độ:* `Cao` = học viên không hoàn thành việc / TA nhận thông tin sai · `Trung bình` = làm được nhưng vướng · `Thấp` = góp ý, không cản việc · `—` = không có vấn đề.
...
| 5 | Nguyễn Duy Phong (HV K4, willing user CP1) | T1 · hỏi cách nộp daily standup | Gõ rất ngắn "nộp daily" → **ANSWER · DOC-03** thẳng, không hỏi lại dù câu chỉ 2 từ | "ok" | — | Giữ nguyên — đúng happy path §6; câu ngắn vẫn không bị ép CLARIFY (lớp lỗi 1 không xảy ra ở đây) |
...
| 7 | Nguyễn Duy Phong | T3 · XP cá nhân | Gõ "Kiểm tra XP hiện tại của tôi" → **ESCALATE · personal_data** + ticket (Đã tra: DOC-01, 09, 04). | "Soạn luôn ticket cũng tiện" | — | Giữ nguyên — non-goal 4 (§4), người thứ 2 liên tiếp được chuyển TA đúng cho câu dữ liệu cá nhân |
| 8 | Nguyễn Duy Phong | T4 · phản hồi câu trả lời sai | Gõ "Nộp dailystand up" → **CLARIFY** "cách nộp hay hạn nộp?" (khác T1: "nộp daily" được ANSWER thẳng — cùng ý, khác nhãn) → chọn chip → ANSWER DOC-03 · bấm "Sai rồi" → **ESCALATE · correction**. **Lỗi lặp lại lần 2:** ticket ghi `Câu hỏi gốc: Sai rồi` | "Phần Bot nhắn lại có lỗi rồi" | **Cao** — cùng lỗi dòng 4, 2/2 người gặp · Thấp — nhãn không ổn định giữa 2 câu gần giống | **Sửa trước demo** (cùng fix dòng 4) · Để dành sau: chạy golden set ≥3 lượt để đo độ ổn định nhãn (eval, Hưởng) |
